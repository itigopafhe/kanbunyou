...
const kuhouSearchBox = document.getElementById('kuhou-search-box'), gokuSearchBox = document.getElementById('goku-search-box');
        ...
        document.addEventListener('click', (e) => {
            ...
            if (target.classList.contains('tr-tag-chip')) { const index = Number(target.dataset.tagIndex); const t = trainingState.tagOptions.kuhou[index]; if (t === undefined) return; const arr = trainingState.settings.tags; const i = arr.indexOf(t); if (i>-1) arr.splice(i,1); else arr.push(t); renderTraining(); return; }
            if (target.classList.contains('tr-goku-tag-chip')) { const index = Number(target.dataset.tagIndex); const t = trainingState.tagOptions.goku[index]; if (t === undefined) return; const arr = trainingState.settings.gokuTags; const i = arr.indexOf(t); if (i>-1) arr.splice(i,1); else arr.push(t); renderTraining(); return; }
            ...
            if (target.classList.contains('tr-choice') && !trainingState.answered) { const q = trainingState.questions[trainingState.idx]; const index = Number(target.dataset.choiceIndex); if (!Number.isInteger(index) || index < 0 || index >= q.choices.length) return; const pick = q.choices[index]; trainingState.lastPickIndex = index; trainingState.lastCorrect = pick === q.answer; if (trainingState.lastCorrect) trainingState.correct++; trainingState.answered = true; renderTraining(); return; }
